...
#define SIZE_POT_PIN A10
#define DAMP_POT_PIN A11
#define MIX_POT_PIN A12
...
void setup() {
  ...
  pinMode(MIX_POT_PIN,INPUT_DISABLE);
  pinMode(SIZE_POT_PIN,INPUT_DISABLE);
  pinMode(DAMP_POT_PIN,INPUT_DISABLE);
...
// Uncomment these lines to adjust parameters with analog inputs
      knob_A1 = (float)analogRead(MIX_POT_PIN) / 1023.0;
      knob_A2 = (float)analogRead(SIZE_POT_PIN) / 1023.0;
      knob_A3 = (float)analogRead(DAMP_POT_PIN) / 1023.0;
